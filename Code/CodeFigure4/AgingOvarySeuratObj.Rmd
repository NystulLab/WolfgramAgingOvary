...
##DotPlot Validation of Clusters (Sup Figure 3A)
```{r}
DotPlot(
  AgingOvary_merge,
  features = c(
...
ggsave("Results/ResultsFigure4/WholeOvaryVolcanoPlot.pdf", width = 10, height = 10)
```
...
##Clustering strength within each age (Sup Figure 3B)
```{r}
library(cluster)
library(dplyr)
library(ggplot2)
...
Volcano_degs <- EnhancedVolcano(deg_list, x = 'avg_log2FC', y = 'p_val_adj', lab = deg_list$Gene, title = 'Volcano Plot for DEGs in Aging pFC', subtitle = bquote('Cutoff values (dashed lines) at LogFC > |0.5| and' ~italic(P[adj])~ '>0.05'), pCutoff = 0.05, FCcutoff = 0.5, border = 'full', gridlines.minor = FALSE, gridlines.major = FALSE, drawConnectors = TRUE, arrowheads = FALSE, pointSize = 3, labSize = 5, widthConnectors = 0, colCustom = keyvals_degs, min.segment.length = 20, titleLabSize = 25, subtitleLabSize = 15, legendLabSize = 15)

Volcano_degs

ggsave("Results/ResultsSupplementals/pFCVolcanoPlot.pdf", width = 10, height = 10)
```
...
##GO Slim for Downregulated Genes in FSCs + pFCs (Figure 4E)
```{r}
library(tidyr)      
library(readr)
library(stringr)    
...
p_slim_fc <- slim_gene_fc |>
  slice_max(n_genes, n = 20) |>
  mutate(Slim_Names = stringr::str_trunc(Slim_Names, 60)) |>
  ggplot(aes(x = n_genes,
             y = forcats::fct_reorder(Slim_Names, n_genes),
             fill = avg_logFC)) +
  geom_col() +
  scale_fill_gradient2(low = "darkblue", high = "skyblue", midpoint = 0) +
  labs(title = "FSC + pFC Downregulated GO-Slim BP Terms",
       x = "# of unique Genes",
       y = "GO-Term",
       fill = "Avg Log2FC") +
...
ggsave("Results/ResultsFigure4/pFCSlimGODownReg.pdf", width = 8, height = 7)
```

##GO Slim Plot for Up and Downregulated Genes in Whole Ovary (Sup Figure 3C)
```{r}
Cell.Markers <- read_csv("Results/ResultsFigure4/WholeOvaryDEG_1v6.csv")
colnames(Cell.Markers)[1] <- "Gene"
